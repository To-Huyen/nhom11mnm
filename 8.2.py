...
class ImageProcessingApp:
    ...
    def create_widgets(self):
        ...
        filter_buttons = [
            ...
            ("Smooth Skin", self.smooth_skin),
            ("Remove Impurities", self.remove_impurities),
    ...
    def blur_background(self):
        if self.processed_image is not None:
            ...
            cv2.imshow("Select ROI for Background Blur", img_rgb)
            cv2.resizeWindow("Select ROI for Background Blur", 400, 400)
            rect = cv2.selectROI("Select ROI for Background Blur", img_rgb, fromCenter=False, showCrosshair=True)
            bgdModel = np.zeros((1, 65), np.float64)
            fgdModel = np.zeros((1, 65), np.float64)
            cv2.grabCut(img_rgb, mask, rect, bgdModel, fgdModel, 5, cv2.GC_INIT_WITH_RECT)
            mask2 = np.where((mask == 2) | (mask == 0), 0, 1).astype('uint8')
            ...
            img_rgb_with_blurred_bg = np.where(mask2[:, :, np.newaxis] == 1, img_rgb_nobg, blurred_bg)

            self.processed_image = cv2.cvtColor(img_rgb_with_blurred_bg, cv2.COLOR_RGB2BGR)
            self.display_images(self.img, self.processed_image)
